...
const Mine = ({ timer, money, energy, setMinePanel, setBoost, url }) => {
  ...
  useEffect(() => {
    try {
      const parsed = JSON.parse(player?.daily);
  ...
  const checkDailyCompletion = async () => {
    try {
      ...
      const response = await fetch(`${url}/api/check-daily`, {
        ...
        body: JSON.stringify({ userId: player?.id, daily: JSON.parse(player?.daily) }),
      });
      if (response.status===200) {
        updateMoney(player.money + reward?.reward); 
        toast.success('Daily reward claimed successfully!', { theme: 'dark' });
        setCards([]);
        setIsWin(true);
        return;
        ...
        toast.warning('Not enough correct cards to complete the daily task', { theme: 'dark' });
      } else if (response.status === 401) {
        toast.error('Unauthorized access. Please login again.', { theme: 'dark' });
      } else if (response.status === 404) {
        toast.error('User not found', { theme: 'dark' });
      } else {
        toast.warning('Unexpected server response', { theme: 'dark' });
      }
    } catch (error) {
      toast.error('Error checking daily completion', { theme: 'dark' });
